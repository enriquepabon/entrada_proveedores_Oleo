    ...
    try:
        ...
        if not clasificacion_data:
            ...
            if os.path.exists(json_path):
                ...
                # Verificar específicamente si tiene datos de clasificación manual
                if 'clasificacion_manual' in clasificacion_data and clasificacion_data['clasificacion_manual']:
                    logger.info(f"Encontrada clasificación manual en el archivo JSON: {clasificacion_data['clasificacion_manual']}")
                else:
                    logger.warning("No se encontró clasificación manual en el archivo JSON")
            else:
                ...
                if os.path.exists(alt_path):
                    ...
                    # Verificar clasificación manual aquí también
                    if 'clasificacion_manual' in clasificacion_data and clasificacion_data['clasificacion_manual']:
                        logger.info(f"Encontrada clasificación manual en archivo alternativo: {clasificacion_data['clasificacion_manual']}")
                else:
                    ...
                    if os.path.exists(third_path):
                        ...
                        # Verificar clasificación manual aquí también
                        if 'clasificacion_manual' in clasificacion_data and clasificacion_data['clasificacion_manual']:
                            logger.info(f"Encontrada clasificación manual en tercera ubicación: {clasificacion_data['clasificacion_manual']}")
                    else:
                ...
                if 'clasificacion_manual' in clasificacion_data:
                    ...
                    for db_campo, posibles_nombres in categoria_mapping.items():
                        for nombre in posibles_nombres:
                            if nombre in manual:
                                valores_manual[f"{db_campo}_manual"] = manual[nombre]
                                logger.info(f"Asignado {db_campo}_manual = {manual[nombre]} (encontrado como '{nombre}')")
                                break
...
def ver_detalles_clasificacion(url_guia):
    ...
    template_data = {
        ...
        'resultados_por_foto': [],
        'fotos_originales': [],
        'fotos_procesadas': [],  # Esta es la variable que usará el template
        'clasificacion_automatica_consolidada': {},
        'raw_data': {},  # Añadir datos crudos para debugging
        'debug_info': {},  # Información de debug
        'json_path': '',  # Path al archivo JSON
        'tiempo_procesamiento': ''  # Tiempo de procesamiento
    }
    ...
    try:
        ...
        if not os.path.exists(clasificacion_path):
            ...
            return render_template('clasificacion/detalles_clasificacion.html', **template_data)
        ...
        with open(clasificacion_path, 'r', encoding='utf-8') as f:
            ...
            # Guardar tiempo de procesamiento si existe
        ...
        # Procesar fotos originales
        if 'fotos' in clasificacion_data:
            fotos_originales = clasificacion_data['fotos']
            logger.info(f"Fotos originales en JSON: {fotos_originales}")
            
            # Tomar solo las primeras 3 fotos que no sean resized o annotated
            fotos_filtradas = []
            for foto in fotos_originales:
                if len(fotos_filtradas) >= 3:
                    break
                if isinstance(foto, str) and 'resized' not in foto and 'annotated' not in foto:
                    fotos_filtradas.append(foto)
            
            # Convertir rutas absolutas a relativas para el navegador
            fotos_relativas = []
            for i, foto in enumerate(fotos_filtradas):
                # Si la ruta es absoluta, extraer solo el nombre del archivo
                if os.path.isabs(foto):
                    # Obtener ruta relativa desde static
                    static_folder = current_app.static_folder
                    if foto.startswith(static_folder):
                        rel_path = foto.replace(static_folder, '').lstrip('/')
                        fotos_relativas.append(url_for('static', filename=rel_path))
                    else:
                        # Extraer solo el nombre de archivo para buscar en static
                        file_name = os.path.basename(foto)
                        # Buscar en ruta estándar de fotos
                        rel_path = f"uploads/fotos/{url_guia}/{file_name}"
                        fotos_relativas.append(url_for('static', filename=rel_path))
                else:
                    # Es una ruta relativa, simplemente agregar url_for
                    fotos_relativas.append(url_for('static', filename=foto))
            
            template_data['fotos_originales'] = fotos_relativas
            logger.info(f"Fotos originales procesadas: {fotos_relativas}")
        
        # Extraer resultados por foto
        resultados_por_foto = []
        ...
        if 'resultados_por_foto' in clasificacion_data:
            ...
            if isinstance(resultados_data, dict):
                # Es un objeto con claves '1', '2', '3', etc.
                keys = sorted([int(k) for k in resultados_data.keys() if k.isdigit()], key=int)
                
                for key in keys:
                    if len(resultados_por_foto) >= 3:  # Limitar a 3 fotos
                        break
                    ...
                    if str_key in resultados_data:
                        ...
                        # Obtener imágenes original y procesada
                        imagen_original = resultado.get('imagen_original', '')
                        imagen_procesada = resultado.get('imagen_procesada', '')
                        
                        # Convertir rutas a URLs para el navegador
                        if imagen_original and os.path.isabs(imagen_original):
                            static_folder = current_app.static_folder
                            if imagen_original.startswith(static_folder):
                                rel_path = imagen_original.replace(static_folder, '').lstrip('/')
                                imagen_original = url_for('static', filename=rel_path)
                            else:
                                file_name = os.path.basename(imagen_original)
                                rel_path = f"uploads/fotos/{url_guia}/{file_name}"
                                # Verificar que el archivo existe antes de asignar la ruta
                                ...
                                if os.path.exists(full_path):
                                    ...
                                    alternative_paths = [
                                        ...
                                        f"uploads/clasificacion/{url_guia}/{file_name}",
                                        f"clasificaciones/fotos/{url_guia}/{file_name}"
                                    ]
                                    for alt_path in alternative_paths:
                                        full_alt_path = os.path.join(current_app.static_folder, alt_path)
                                        if os.path.exists(full_alt_path):
                                            imagen_original = url_for('static', filename=alt_path)
                                            logger.info(f"Imagen original encontrada en ruta alternativa: {alt_path}")
                                            break
                        
                        if imagen_procesada and os.path.isabs(imagen_procesada):
                            static_folder = current_app.static_folder
                            if imagen_procesada.startswith(static_folder):
                                rel_path = imagen_procesada.replace(static_folder, '').lstrip('/')
                                imagen_procesada = url_for('static', filename=rel_path)
                            else:
                                file_name = os.path.basename(imagen_procesada)
                                rel_path = f"uploads/fotos/{url_guia}/{file_name}"
                                # Verificar que el archivo existe antes de asignar la ruta
                                full_path = os.path.join(current_app.static_folder, rel_path)
                                if os.path.exists(full_path):
                                    ...
                                    for alt_path in alternative_paths:
                                        full_alt_path = os.path.join(current_app.static_folder, alt_path)
                                        if os.path.exists(full_alt_path):
                                            imagen_procesada = url_for('static', filename=alt_path)
                                            logger.info(f"Imagen procesada encontrada en ruta alternativa: {alt_path}")
                                            break
                        
                        # Si no hay imagen procesada, buscar una posible versión _annotated
                        if not imagen_procesada and imagen_original:
                            base_name = os.path.basename(imagen_original)
                            name, ext = os.path.splitext(base_name)
                            annotated_name = f"{name}_annotated{ext}"
                            
                            # Buscar el archivo _annotated en varias ubicaciones posibles
                            possible_paths = [
                                f"uploads/fotos/{url_guia}/{annotated_name}",
                                f"fotos_racimos_temp/{url_guia}/{annotated_name}",
                                f"uploads/clasificacion/{url_guia}/{annotated_name}",
                                f"clasificaciones/fotos/{url_guia}/{annotated_name}"
                            ]
                            
                            for path in possible_paths:
                                full_path = os.path.join(current_app.static_folder, path)
                                if os.path.exists(full_path):
                                    imagen_procesada = url_for('static', filename=path)
                                    logger.info(f"Imagen anotada encontrada en: {path}")
                                    break
                    
                        # Preparar conteo por categorías
                        categorias = {
                            'verdes': {'cantidad': 0, 'porcentaje': 0.0},
                            'maduros': {'cantidad': 0, 'porcentaje': 0.0},
                            'sobremaduros': {'cantidad': 0, 'porcentaje': 0.0},
                            'danio_corona': {'cantidad': 0, 'porcentaje': 0.0},
                            'pendunculo_largo': {'cantidad': 0, 'porcentaje': 0.0},
                            'podridos': {'cantidad': 0, 'porcentaje': 0.0}
                        }
                        ...
                        for det in detecciones:
                            ...
                            if 'verde' in clase:
                                categorias['verdes']['cantidad'] += 1
                            elif 'maduro' in clase and not 'sobre' in clase:
                                categorias['maduros']['cantidad'] += 1
                            elif 'sobremaduro' in clase or 'sobre maduro' in clase:
                                categorias['sobremaduros']['cantidad'] += 1
                            elif 'danio' in clase or 'daño' in clase or 'corona' in clase:
                                categorias['danio_corona']['cantidad'] += 1
                            elif 'pendunculo' in clase or 'pedunculo' in clase:
                                categorias['pendunculo_largo']['cantidad'] += 1
                            elif 'podrido' in clase:
                                categorias['podridos']['cantidad'] += 1
                        
                        # Calcular porcentajes
                        if total_detecciones > 0:
                            for cat in categorias:
                                if categorias[cat]['cantidad'] > 0:
                                    categorias[cat]['porcentaje'] = round((categorias[cat]['cantidad'] / total_detecciones) * 100, 1)
                        
                        # Agregar resultado procesado
                        resultados_por_foto.append({
                            'numero': key,
                            'detecciones': detecciones,
                ...
                for i, resultado in enumerate(resultados_data[:3]):  # Limitar a 3 fotos
                    # Extraer detecciones
                    detecciones = resultado.get('detecciones', [])
        ...
        # Si no tenemos resultados_por_foto pero sí tenemos fotos, crear entradas básicas
        if not resultados_por_foto and len(template_data['fotos_originales']) > 0:
            for i, foto_original in enumerate(template_data['fotos_originales'][:3]):
                # Buscar si existe versión anotada
                base_name = os.path.basename(foto_original)
                name, ext = os.path.splitext(base_name)
                annotated_name = f"{name}_annotated{ext}"
        ...
        # Asignar los resultados procesados al template
        template_data['fotos_procesadas'] = resultados_por_foto
        
        # Extraer clasificación consolidada
        if 'clasificacion_automatica' in clasificacion_data:
        ...
        logger.error(f"Error en ver_detalles_clasificacion: {str(e)}")
        logger.error(traceback.format_exc())
...
def generate_annotated_image(original_image_path, detections, output_path=None):
    ...
    try:
        ...
        colors = {
            ...
            'danio_corona': (128, 0, 128), # Morado
            'pendunculo_largo': (0, 128, 128), # Cian oscuro
            'podrido': (0, 0, 0)       # Negro
        }
        ...
        for detection in detections:
            # Obtener coordenadas normalizadas y convertirlas a píxeles
            x = float(detection.get('x', 0))
            y = float(detection.get('y', 0))
            width = float(detection.get('width', 0))
            height = float(detection.get('height', 0))
            
            # Ajustar según el formato de las coordenadas (centrado vs esquina)
            # Si las coordenadas están normalizadas (0-1), convertir a píxeles
            ...
            # Calcular esquinas del rectángulo (x,y es el centro del rectángulo)
            x1 = x - width / 2
            y1 = y - height / 2
            x2 = x + width / 2
            y2 = y + height / 2
            
            # Obtener clase y determinar color
            detection_class = detection.get('class', '').lower()
            color = (255, 0, 0)  # Rojo por defecto
            
            # Buscar color apropiado
            for class_key, class_color in colors.items():
                if class_key in detection_class:
                    color = class_color
                    break
