...
def clasificacion(codigo):
    ...
    try:
        ...
        # Verificar si debemos respetar el código original (enviado desde guía centralizada)
        respetar_codigo = request.args.get('respetar_codigo', 'false').lower() == 'true'
        referrer = request.referrer or ""
        desde_guia_centralizada = 'guia-centralizada' in referrer
        
        # Si la petición viene de guía centralizada o el parámetro respetar_codigo está activo,
        # no buscar un código más reciente
        if respetar_codigo or desde_guia_centralizada:
            codigo_guia_json = codigo
            logger.info(f"Respetando código original debido a: respetar_codigo={respetar_codigo}, desde_guia_centralizada={desde_guia_centralizada}")
        else:
...
@bp.route('/guardar_clasificacion_final/<path:codigo_guia>')
def guardar_clasificacion_final(codigo_guia):
    """
    Guarda la clasificación como completada y redirige a la guía centralizada.
    Esta función marca la clasificación como finalizada en la guía y redirige al usuario.
    """
    try:
        # Normalizar el código de guía para evitar duplicados
        codigo_guia = codigo_guia.strip()
        logger.info(f"Guardando clasificación final para {codigo_guia}")
        ...
        if not datos_guia:
            ...
            flash("No se encontraron datos para la guía", "danger")
            return redirect(url_for('misc.index'))
        
        # Verificar si ya existe un codigo_guia en los datos y usarlo para mantener consistencia
        if datos_guia.get('codigo_guia') and datos_guia.get('codigo_guia') != codigo_guia:
            logger.info(f"Se encontró un código de guía diferente en los datos: {datos_guia['codigo_guia']}. Se utilizará este para mantener consistencia.")
            # No cambiar código_guia aquí, para mantener la URL a la que redirigimos
        
        # Obtener datos de clasificación (manual y automática)
        clasificacion_data = None
        try:
            from db_operations import get_clasificacion_by_codigo_guia
            clasificacion_data = get_clasificacion_by_codigo_guia(codigo_guia)
            logger.info(f"Datos de clasificación encontrados en base de datos: {clasificacion_data is not None}")
        except Exception as e:
            logger.error(f"Error al buscar clasificación en base de datos: {str(e)}")
        
        # Si no se encontró en la base de datos, buscar en archivos
        if not clasificacion_data:
            clasificaciones_dir = os.path.join(current_app.static_folder, 'clasificaciones')
            json_path = os.path.join(clasificaciones_dir, f"clasificacion_{codigo_guia}.json")
            ...
            if os.path.exists(json_path):
                ...
                logger.warning(f"No se encontró archivo de clasificación: {json_path}")
        
        # Si no hay datos de clasificación, inicializar un diccionario vacío
        if not clasificacion_data:
            # Si no hay datos, crear una estructura mínima
            ...
            clasificacion_data = {
                ...
                'fecha_clasificacion': fecha_actual,
                'hora_clasificacion': hora_actual,
                'clasificacion_manual': datos_guia.get('clasificacion_manual', {}),
                'clasificacion_automatica': datos_guia.get('clasificacion_automatica', {}),
                'estado': 'completado'
            }
            
            logger.info(f"Creada estructura de clasificación mínima para {codigo_guia}")
        
        # Asegurarse de que el estado esté configurado como completado
        clasificacion_data['estado'] = 'completado'
        
        # Guardar en la base de datos
        try:
            from db_operations import store_clasificacion
            store_clasificacion(clasificacion_data)
            logger.info(f"Clasificación guardada en base de datos para {codigo_guia}")
        except Exception as e:
            logger.error(f"Error al guardar clasificación en base de datos: {str(e)}")
            
            # Si falla la base de datos, guardar en archivo
            try:
                clasificaciones_dir = os.path.join(current_app.static_folder, 'clasificaciones')
                json_path = os.path.join(clasificaciones_dir, f"clasificacion_{codigo_guia}.json")
                ...
                logger.error(f"Error al guardar clasificación en archivo: {str(e2)}")
                flash("Error al guardar la clasificación", "danger")
                return redirect(url_for('clasificacion.ver_resultados_clasificacion', url_guia=codigo_guia))
        
        # Actualizar estado de la guía para marcar la clasificación como completada
        datos_guia['estado_actual'] = 'clasificacion_completada'
        datos_guia['clasificacion_completada'] = True
        datos_guia['estado_clasificacion'] = 'completado'
        datos_guia['fecha_clasificacion'] = clasificacion_data.get('fecha_clasificacion', datetime.now().strftime('%d/%m/%Y'))
        datos_guia['hora_clasificacion'] = clasificacion_data.get('hora_clasificacion', datetime.now().strftime('%H:%M:%S'))
        
        # Asegurarse de que la clasificación esté en pasos_completados
        if 'pasos_completados' not in datos_guia:
            datos_guia['pasos_completados'] = ['entrada', 'pesaje']
        
        if 'clasificacion' not in datos_guia['pasos_completados']:
            datos_guia['pasos_completados'].append('clasificacion')
            logger.info(f"Agregada 'clasificacion' a pasos_completados: {datos_guia['pasos_completados']}")
        
        # Asegurar que clasificacion esté en datos_disponibles
        if 'datos_disponibles' not in datos_guia:
            datos_guia['datos_disponibles'] = ['entrada', 'pesaje']
        
        if 'clasificacion' not in datos_guia['datos_disponibles']:
            datos_guia['datos_disponibles'].append('clasificacion')
            logger.info(f"Agregada 'clasificacion' a datos_disponibles: {datos_guia['datos_disponibles']}")
        
        # Si hay pasos pendientes, actualizar para reflejar que la clasificación está completada
        if 'pasos_pendientes' in datos_guia and isinstance(datos_guia['pasos_pendientes'], list):
            if 'clasificacion' in datos_guia['pasos_pendientes']:
                datos_guia['pasos_pendientes'].remove('clasificacion')
            if 'Clasificacion' in datos_guia['pasos_pendientes']:
                datos_guia['pasos_pendientes'].remove('Clasificacion')
        
        # Actualizar porcentaje de avance si existe
        if 'porcentaje_avance' in datos_guia:
            datos_guia['porcentaje_avance'] = 60  # 60% completado
            logger.info(f"Actualizado porcentaje de avance a 60%")
        
        # Guardar los datos actualizados de la guía
        utils.update_datos_guia(codigo_guia, datos_guia)
        logger.info(f"Datos de guía actualizados para {codigo_guia} con clasificación completada")
        logger.info(f"Estado final de la guía: estado_actual={datos_guia.get('estado_actual')}, clasificacion_completada={datos_guia.get('clasificacion_completada')}")
        
        # Actualizar DIRECTAMENTE la tabla pesajes_bruto, como se hace en el módulo de pesaje
        try:
            from db_operations import update_pesaje_bruto
            
            # Preparar datos para actualizar en el mismo formato usado en pesaje_bruto
            datos_pesaje = {
                'estado_actual': 'clasificacion_completada',
                'clasificacion_completada': 1,  # Usar entero para compatibilidad con SQLite
                'fecha_clasificacion': datos_guia.get('fecha_clasificacion'),
                'hora_clasificacion': datos_guia.get('hora_clasificacion'),
                'estado_clasificacion': 'completado'
            }
            
            # Intentar actualizar en la tabla de pesajes_bruto
            resultado_actualizacion = update_pesaje_bruto(codigo_guia, datos_pesaje)
            logger.info(f"Actualización directa en pesajes_bruto: {resultado_actualizacion}")
            
            # También actualizar en 'app/utils/db_operations.py' que maneja ambas bases de datos
            try:
                from app.utils import db_operations as app_db_ops
                if hasattr(app_db_ops, 'update_pesaje_bruto'):
                    app_db_ops.update_pesaje_bruto(codigo_guia, datos_pesaje)
                    logger.info("Actualización realizada también mediante app.utils.db_operations")
            except Exception as app_db_err:
                logger.error(f"Error al actualizar mediante app.utils.db_operations: {str(app_db_err)}")
            
        except ImportError:
            logger.error("No se pudo importar update_pesaje_bruto de db_operations")
        except Exception as update_error:
            logger.error(f"Error actualizando directamente pesajes_bruto: {str(update_error)}")
        
        # Mantener la actualización existente para compatibilidad
        try:
            # Actualización forzada en ambas bases de datos
            logger.info("Intentando actualización explícita en ambas bases de datos")
            
            for db_path in ['database.db', 'tiquetes.db']:
                try:
                    ...
                    logger.info(f"Columnas disponibles en {db_path}: {columnas}")
                    ...
                    if exists:
                        # Construir la actualización basada en las columnas disponibles
                        updates = []
                        params = []
                        
                        if 'estado_actual' in columnas:
                            updates.append("estado_actual = ?")
                            params.append('clasificacion_completada')
                        
                        if 'clasificacion_completada' in columnas:
                            updates.append("clasificacion_completada = ?")
                            params.append(1)
                        
                        if 'fecha_clasificacion' in columnas:
                            updates.append("fecha_clasificacion = ?")
                            params.append(datos_guia.get('fecha_clasificacion'))
                        
                        if 'hora_clasificacion' in columnas:
                            updates.append("hora_clasificacion = ?")
                            params.append(datos_guia.get('hora_clasificacion'))
                        
                        # Si no existen las columnas específicas, crear un campo genérico
                        if 'datos_json' in columnas:
                            # Guardar todos los datos necesarios en el campo JSON
                            try:
                                cursor.execute("SELECT datos_json FROM pesajes_bruto WHERE codigo_guia = ?", (codigo_guia,))
                                datos_json_str = cursor.fetchone()[0]
                                if datos_json_str:
                                ...
                                updates.append("datos_json = ?")
                                params.append(json.dumps(datos_json))
                            except Exception as json_err:
                                logger.error(f"Error manipulando datos_json: {str(json_err)}")
                        
                        # Si hay actualizaciones para hacer...
                        if updates:
                            params.append(codigo_guia)  # Para la condición WHERE
