...
def pesaje(codigo):
    ...
    try:
        ...
        if datos_guia.get('estado_actual') in ['clasificacion_completada', 'pesaje_tara_completado', 'registro_completado']:
            # En lugar de mostrar un error, generar un nuevo código de guía
            logger.info(f"La guía {codigo_guia_completo} ya ha sido procesada. Generando nueva guía para el proveedor {codigo_base}")
            
            # Limpiar datos de sesión relacionados con la guía anterior
            session_keys_to_reset = ['estado_actual', 'peso_bruto', 'tipo_pesaje', 'fecha_pesaje', 'hora_pesaje']
            for key in session_keys_to_reset:
                if key in session:
                    del session[key]
            
            # Generar un nuevo código de guía usando la función de utils
            nuevo_codigo_guia = utils.generar_codigo_guia(codigo_base)
            
            # Guardar el nuevo código en la sesión
            session['codigo_guia'] = nuevo_codigo_guia
            session.modified = True
            
            # Redirigir a la URL correcta con el nuevo código
            return redirect(url_for('pesaje', codigo=nuevo_codigo_guia))
...
def registrar_peso_directo():
    ...
    try:
        ...
        datos_guia = utils.get_datos_guia(codigo_guia)
        codigo_proveedor = datos_guia.get('codigo')
        nombre_proveedor = datos_guia.get('nombre')
        ...
        store_pesaje_bruto(datos_pesaje)
...
def registrar_peso_virtual():
    """
    Registra el peso bruto manual o virtual y lo almacena en la base de datos SQLite.
...
def registrar_clasificacion():
    """
    Registra la clasificación manual de racimos y envía a procesamiento automático.
    """
    try:
        codigo_guia = request.form.get('codigo_guia')
        if not codigo_guia:
        ...
        # Iniciar procesamiento con Roboflow en segundo plano si hay fotos
        if fotos_paths:
            logger.info(f"Iniciando procesamiento con Roboflow para {len(fotos_paths)} fotos: {fotos_paths}")
            from concurrent.futures import ThreadPoolExecutor
            executor = ThreadPoolExecutor(max_workers=1)
            executor.submit(process_images_with_roboflow, codigo_guia_a_usar, fotos_paths, guia_fotos_dir, json_path)
            logger.info(f"Iniciado procesamiento automático para guía: {codigo_guia_a_usar}")
        else:
            logger.warning(f"No se encontraron fotos para procesar con Roboflow para la guía: {codigo_guia_a_usar}")
...
def ver_resultados_clasificacion(url_guia):
    try:
        ...
        # Asegurar que tenemos datos de clasificación manual
        if not clasificacion_data.get('clasificacion_manual'):
            logger.warning(f"Datos de clasificación manual no encontrados para {url_guia}")
            # Intentar recuperar de otra clave si es necesario
            if 'clasificaciones' in clasificacion_data and isinstance(clasificacion_data['clasificaciones'], dict):
                if 'manual' in clasificacion_data['clasificaciones']:
                    clasificacion_data['clasificacion_manual'] = clasificacion_data['clasificaciones']['manual']
                    logger.info("Recuperados datos de clasificación manual desde 'clasificaciones.manual'")
        
        # Verificar y corregir la estructura de clasificación automática
        if 'clasificacion_automatica' in clasificacion_data:
            logger.info(f"Verificando estructura de clasificacion_automatica: {clasificacion_data['clasificacion_automatica']}")
            # Asegurar que todas las categorías tienen la estructura correcta
            categorias_esperadas = ['verdes', 'maduros', 'sobremaduros', 'danio_corona', 'pendunculo_largo', 'podridos']
            
            # Si clasificacion_automatica no es un diccionario, inicializarlo
            if not isinstance(clasificacion_data['clasificacion_automatica'], dict):
                clasificacion_data['clasificacion_automatica'] = {}
            
            # Asegurar que todas las categorías existen con la estructura correcta
            for categoria in categorias_esperadas:
                if categoria not in clasificacion_data['clasificacion_automatica']:
                    clasificacion_data['clasificacion_automatica'][categoria] = {"cantidad": 0, "porcentaje": 0}
                elif not isinstance(clasificacion_data['clasificacion_automatica'][categoria], dict):
                    clasificacion_data['clasificacion_automatica'][categoria] = {"cantidad": 0, "porcentaje": 0}
                elif "cantidad" not in clasificacion_data['clasificacion_automatica'][categoria]:
                    clasificacion_data['clasificacion_automatica'][categoria]["cantidad"] = 0
                elif "porcentaje" not in clasificacion_data['clasificacion_automatica'][categoria]:
                    clasificacion_data['clasificacion_automatica'][categoria]["porcentaje"] = 0
            
            # Recalcular porcentajes para asegurar consistencia
            total_racimos = sum(cat.get("cantidad", 0) for cat in clasificacion_data['clasificacion_automatica'].values() if isinstance(cat, dict))
            
            if total_racimos > 0:
                for categoria in clasificacion_data['clasificacion_automatica']:
                    if isinstance(clasificacion_data['clasificacion_automatica'][categoria], dict):
                        cantidad = clasificacion_data['clasificacion_automatica'][categoria].get("cantidad", 0)
                        clasificacion_data['clasificacion_automatica'][categoria]["porcentaje"] = round(
                            (cantidad / total_racimos) * 100, 2
                        ) if cantidad > 0 else 0
            
            # Agregar total_racimos_detectados si no existe
            if 'total_racimos_detectados' not in clasificacion_data:
                clasificacion_data['total_racimos_detectados'] = total_racimos
        else:
            # Si no existe clasificacion_automatica, crear estructura vacía
            clasificacion_data['clasificacion_automatica'] = {
                "verdes": {"cantidad": 0, "porcentaje": 0},
                "maduros": {"cantidad": 0, "porcentaje": 0},
                "sobremaduros": {"cantidad": 0, "porcentaje": 0},
            ...
            clasificacion_data['total_racimos_detectados'] = 0
...
def generar_pdf_clasificacion(codigo_guia):
    """
    Genera un PDF con los resultados de la clasificación para una guía.
    """
    try:
        ...
        if not clasificacion_data:
            logger.error(f"Clasificación no encontrada en la base de datos para código: {codigo_guia}")
        ...
        if os.path.exists(json_path):
            ...
            flash("Clasificación no encontrada", "error")
            return redirect(url_for('index'))
                
        # Obtener datos de la guía
        datos_guia = utils.get_datos_guia(codigo_guia)
        if not datos_guia:
            flash("Datos de guía no encontrados", "error")
            return redirect(url_for('index'))
        
        # Generar QR para la guía
        ...
        # Preparar datos para fotos de clasificación
        fotos_clasificacion = []
        if 'fotos' in clasificacion_data and clasificacion_data['fotos']:
            fotos_clasificacion = clasificacion_data['fotos']
            
        # Filtrar fotos válidas
        valid_fotos = []
        if 'id' in clasificacion_data:
            id_clasificacion = clasificacion_data['id']
            for i in range(1, 4):  # Permitimos hasta 3 fotos
                foto_original = f"clasificaciones/fotos/{id_clasificacion}/foto_{i}.jpg"
                foto_procesada = f"clasificaciones/fotos/{id_clasificacion}/foto_{i}_procesada.jpg"
                
                # Verificar si las fotos existen
                if os.path.exists(os.path.join(app.root_path, 'static', foto_original)):
                    valid_fotos.append({
                        'original': url_for('static', filename=foto_original, _external=True),
                        'procesada': url_for('static', filename=foto_procesada, _external=True) if 
                                   os.path.exists(os.path.join(app.root_path, 'static', foto_procesada)) else None
                    })
        
        # Preparar clasificación manual
        clasificacion_manual = clasificacion_data.get('clasificacion_manual', {})
        
        # Preparar clasificación automática
        clasificacion_automatica = clasificacion_data.get('clasificacion_automatica', {
            "verdes": {"cantidad": 0, "porcentaje": 0},
            "maduros": {"cantidad": 0, "porcentaje": 0},
            "sobremaduros": {"cantidad": 0, "porcentaje": 0},
        ...
        # Calcular totales para clasificación manual
        total_racimos = int(datos_guia.get('cantidad_racimos', 0))
        ...
        html = render_template('clasificacion_pdf_template.html',
            ...
            nombre_proveedor=datos_guia.get('nombre_agricultor', datos_guia.get('nombre', 'No disponible')),
            fecha_clasificacion=clasificacion_data.get('fecha_clasificacion', fecha_generacion),
            hora_clasificacion=clasificacion_data.get('hora_clasificacion', hora_generacion),
        ...
        pdf_filename = f"clasificacion_{codigo_guia}_{now.strftime('%Y%m%d%H%M%S')}.pdf"
...
def ver_resultados_automaticos(url_guia):
    try:
        logger.info(f"Mostrando página de clasificación automática para guía: {url_guia}")
        ...
        if not os.path.exists(json_path):
            logger.error(f"Archivo JSON no encontrado para {url_guia}: {json_path}")
            return render_template('error.html', message="Archivo de clasificación no encontrado")
            
        # Leer los datos del archivo JSON para obtener información básica
        with open(json_path, 'r') as f:
            try:
                clasificacion_data = json.load(f)
                logger.info(f"Leído archivo JSON con éxito para guía {url_guia}. Claves: {list(clasificacion_data.keys())}")
            except json.JSONDecodeError as e:
                logger.error(f"Error decodificando JSON: {str(e)}")
                return render_template('error.html', message=f"Error al leer el archivo de clasificación: {str(e)}")
        
        # Obtener datos adicionales de la guía
        datos_guia = utils.get_datos_guia(url_guia)
        if not datos_guia:
            logger.warning(f"No se encontraron datos para la guía: {url_guia}")
            datos_guia = {'nombre': 'No disponible', 'codigo_proveedor': url_guia.split('_')[0] if '_' in url_guia else url_guia}
        
        # Buscar fotos disponibles para procesamiento
        fotos_dir = os.path.join(app.static_folder, 'clasificaciones', 'fotos', url_guia)
        fotos_temp_dir = os.path.join(app.static_folder, 'fotos_racimos_temp', url_guia)
        
        fotos_disponibles = []
        
        # Verificar en directorio principal de fotos
        if os.path.exists(fotos_dir):
            for archivo in sorted(os.listdir(fotos_dir)):
                if archivo.startswith('foto_') and archivo.endswith('.jpg') and not archivo.endswith('_procesada.jpg') and not archivo.endswith('_etiquetas.jpg'):
                    fotos_disponibles.append(os.path.join(fotos_dir, archivo))
        
        # Verificar en directorio temporal
        if os.path.exists(fotos_temp_dir):
            for archivo in sorted(os.listdir(fotos_temp_dir)):
                if archivo.startswith('foto_') and archivo.endswith('.jpg') and not archivo.endswith('_procesada.jpg') and not archivo.endswith('_etiquetas.jpg'):
                    fotos_disponibles.append(os.path.join(fotos_temp_dir, archivo))
        
        # Convertir rutas absolutas a relativas
        fotos_relativas = []
        for foto in fotos_disponibles:
            if foto.startswith(app.static_folder):
                fotos_relativas.append(foto[len(app.static_folder):].lstrip('/'))
        
        # Verificar si ya hay resultados de clasificación automática
        tiene_resultados = False
        if 'clasificacion_automatica' in clasificacion_data:
            # Verificar si hay alguna categoría con valor mayor a 0
            for categoria, datos in clasificacion_data['clasificacion_automatica'].items():
                if isinstance(datos, dict) and datos.get('cantidad', 0) > 0:
        ...
        template_data = {
            ...
            'cantidad_total_racimos': datos_guia.get('cantidad_racimos', 0),
            'fotos_disponibles': fotos_relativas,
            'cant_fotos': len(fotos_relativas),
            'tiene_resultados': tiene_resultados,
            'json_path': json_path.replace(app.static_folder, '').lstrip('/')
        }
        
        logger.info(f"Enviando a template - cantidad_total_racimos: {template_data['cantidad_total_racimos']}")
        logger.info(f"Enviando a template - cant_fotos: {template_data['cant_fotos']}")
        logger.info(f"Enviando a template - tiene_resultados: {template_data['tiene_resultados']}")
        
        return render_template('auto_clasificacion_inicio.html', **template_data)
    except Exception as e:
        logger.error(f"Error en ver_resultados_automaticos: {str(e)}")
        logger.error(traceback.format_exc())
...
def ver_resultados_pesaje(codigo_guia):
    try:
        ...
        # Si la URL no tiene el código guía completo y es diferente al código en la sesión,
        # solo redirigir si es necesario para evitar bucles de redirección
        sesion_codigo_guia = session.get('codigo_guia', '')
        if codigo != codigo_guia_completo and codigo != sesion_codigo_guia:
            logger.info(f"Redireccionando de URL con código {codigo} a URL con código {codigo_guia_completo}")
            return redirect(url_for('ver_resultados_pesaje', codigo_guia=codigo_guia_completo))

        # Verificar que el pesaje esté completado
        # Si el estado en la sesión es 'pesaje_completado', consideramos que la guía está procesada
        # aunque la guía especifica no tenga ese estado
        if (datos_guia.get('estado_actual') != 'pesaje_completado' and 
            estado_actual_session != 'pesaje_completado'):
            flash("El pesaje no ha sido completado para esta guía.", "warning")
            return redirect(url_for('pesaje', codigo=codigo_guia_completo))
        
        # Asegurar que el estado se actualice a pesaje_completado en la sesión
        session['estado_actual'] = 'pesaje_completado'
        session.modified = True
        
        # Asegurar que el estado se actualice a pesaje_completado en los datos de la guía
        # y guardar los cambios
        if datos_guia.get('estado_actual') != 'pesaje_completado':
            datos_guia['estado_actual'] = 'pesaje_completado'
            
            # Guardar los cambios en el archivo de la guía
            guia_path = os.path.join(guias_folder, f"guia_{codigo_guia_completo}.html")
            if os.path.exists(guia_path):
                with open(guia_path, 'w', encoding='utf-8') as f:
                    json.dump(datos_guia, f)
                logger.info(f"Estado de la guía actualizado a 'pesaje_completado' en {guia_path}")
...
def lista_pesajes():
    ...
    try:
        ...
        from db_operations import get_pesajes_bruto
...
def registrar_peso_neto_directo():
    try:
        ...
        if not codigo_guia or not peso_neto or not peso_bruto or not peso_producto:
            return jsonify({
                'success': False,
                'message': 'Faltan datos requeridos'
            })
        ...
        if datos_guia and datos_guia.get('estado_actual') in ['pesaje_tara_completado', 'registro_completado']:
            ...
            # Extraer el código base del proveedor
            codigo_base = codigo_guia.split('_')[0] if '_' in codigo_guia else codigo_guia
            
            # Limpiar datos de sesión relacionados con la guía anterior
            session_keys_to_reset = ['estado_actual', 'peso_bruto', 'tipo_pesaje', 'fecha_pesaje', 'hora_pesaje', 
                                    'peso_neto', 'peso_producto', 'tipo_pesaje_neto', 'hora_pesaje_neto', 'fecha_pesaje_neto']
            for key in session_keys_to_reset:
                if key in session:
                    del session[key]
            
            # Generar un nuevo código de guía para el mismo proveedor
            nuevo_codigo_guia = utils.generar_codigo_guia(codigo_base)
            ...
            # Devolver respuesta con el nuevo código
            return jsonify({
                'success': True,
                'message': 'Se ha generado una nueva guía para el proveedor',
                'nuevo_codigo': nuevo_codigo_guia,
                'redirect': url_for('pesaje', codigo=nuevo_codigo_guia)
            })
        
        # Validar que el peso neto sea menor al peso bruto
        try:
            if float(peso_neto) >= float(peso_bruto):
                return jsonify({
                    'success': False,
                    'message': 'El peso neto debe ser menor al peso bruto'
                })
        except ValueError:
            return jsonify({
                'success': False,
                'message': 'Los pesos deben ser valores numéricos'
            })
        
        # Registrar el peso neto en el sistema
        now = datetime.now()
        session['peso_neto'] = peso_neto
        session['peso_producto'] = peso_producto
        ...
        # Guardar la foto de la báscula
        foto_bascula = None
        if 'foto_bascula' in request.files:
            foto = request.files['foto_bascula']
            if foto and foto.filename:
                # Generar nombre de archivo seguro
                filename = secure_filename(f"{codigo_guia}_bascula_{now.strftime('%Y%m%d%H%M%S')}.jpg")
                filepath = os.path.join(fotos_pesaje_neto_dir, filename)
                foto.save(filepath)
                foto_bascula = os.path.join('static', 'fotos_pesaje_neto', codigo_guia, filename)
        
        # Guardar las fotos de soporte
        fotos_guardadas = []
        for i in range(1, 5):
            foto_key = f'foto_soporte_{i}'
            if foto_key in request.files:
                foto = request.files[foto_key]
                if foto and foto.filename:
                    # Generar nombre de archivo seguro
                    filename = secure_filename(f"{codigo_guia}_soporte_{i}_{now.strftime('%Y%m%d%H%M%S')}.jpg")
                    filepath = os.path.join(fotos_pesaje_neto_dir, filename)
                    foto.save(filepath)
                    fotos_guardadas.append(os.path.join('static', 'fotos_pesaje_neto', codigo_guia, filename))
        
        # Almacenar las rutas de las fotos en la sesión
        session['foto_bascula_neto'] = foto_bascula
        session['fotos_pesaje_neto'] = fotos_guardadas
        
        # Enviar datos al webhook de registro de peso neto
        response = requests.post(
            ...
            json={
                ...
                'foto_bascula_neto': foto_bascula,
                'fotos_pesaje_neto': fotos_guardadas
            }
        )
        
        if response.status_code == 200:
            # Actualizar los datos de la guía con el peso neto
            datos_guia = utils.get_datos_guia(codigo_guia)
            datos_guia['peso_neto'] = peso_neto
            datos_guia['peso_producto'] = peso_producto
            datos_guia['tipo_pesaje_neto'] = 'directo'
            datos_guia['fecha_pesaje_neto'] = now.strftime('%d/%m/%Y')
            datos_guia['hora_pesaje_neto'] = now.strftime('%H:%M:%S')
            datos_guia['foto_bascula_neto'] = foto_bascula
            datos_guia['fotos_pesaje_neto'] = fotos_guardadas
            
            # Guardar datos actualizados de la guía
            utils.update_datos_guia(codigo_guia, datos_guia)
            
            return jsonify({
                'success': True,
                'message': 'Peso neto registrado correctamente',
                'redirect_url': url_for('ver_resultados_pesaje_neto', codigo_guia=codigo_guia)
            })
        else:
            return jsonify({
                'success': False,
                'message': f'Error al registrar el peso neto: {response.text}'
            })
    except Exception as e:
        logger.error(f"Error registrando peso neto directo: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({
            'success': False,
...
def registrar_peso_neto_virtual():
    try:
        ...
        logger.info(f"Iniciando registro de peso neto virtual para guía: {codigo_guia}, peso neto: {peso_neto}")
        
        if not codigo_guia or not peso_neto or not peso_bruto or not peso_producto:
            logger.error(f"Faltan datos requeridos: codigo_guia={codigo_guia}, peso_neto={peso_neto}")
            return jsonify({
                'success': False,
                'message': 'Faltan datos requeridos'
            })
        ...
        session['fecha_pesaje_neto'] = now.strftime('%d/%m/%Y')
        session['fotos_pesaje_neto'] = fotos_guardadas
        session['comentarios_neto'] = comentarios
        
        logger.info(f"  peso_neto: {session.get('peso_neto')}")
        ...
        if response.status_code == 200:
            ...
            datos_guia['peso_neto'] = peso_neto
            datos_guia['peso_producto'] = peso_producto
            datos_guia['tipo_pesaje_neto'] = 'virtual'
            datos_guia['fecha_pesaje_neto'] = now.strftime('%d/%m/%Y')
            datos_guia['hora_pesaje_neto'] = now.strftime('%H:%M:%S')
            datos_guia['fotos_pesaje_neto'] = fotos_guardadas
            datos_guia['comentarios_neto'] = comentarios
            ...
            logger.error(f"Error en respuesta del webhook: {response.status_code}, {response.text}")
            return jsonify({
                'success': False,
                'message': f'Error al registrar el peso neto: {response.text}'
            })
    except Exception as e:
        logger.error(f"Error en registro de peso neto virtual: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({
            'success': False,
            'message': str(e)
...
def registrar_peso_neto():
    """
    Registra el peso neto (tara) y lo almacena en la base de datos SQLite.
    Puede recibir datos en formato JSON o form-data (con imagen).
    """
    try:
        # Verificar si los datos vienen en JSON o en form-data
        if request.is_json:
            ...
            if 'imagen' in request.files and request.files['imagen'].filename:
                imagen = request.files['imagen']
                # Crear directorio si no existe
                img_dir = os.path.join(app.static_folder, 'pesajes_neto', codigo_guia)
                os.makedirs(img_dir, exist_ok=True)
                
                # Guardar imagen
                filename = f"pesaje_neto_{utils.generate_unique_id()}.jpg"
                file_path = os.path.join(img_dir, filename)
                imagen.save(file_path)
                
                # Guardar ruta relativa para acceso web
                imagen_path = os.path.join('static', 'pesajes_neto', codigo_guia, filename)
        
        # Validar datos requeridos
        if not codigo_guia or not peso_tara:
            return jsonify({'success': False, 'message': 'Faltan datos requeridos: código de guía y peso tara'})
        ...
        # Calcular peso neto (peso bruto - tara)
        try:
            peso_bruto_float = float(peso_bruto) if peso_bruto else 0
            peso_tara_float = float(peso_tara) if peso_tara else 0
            peso_neto = max(0, peso_bruto_float - peso_tara_float)
        except ValueError:
            logger.error(f"Error al convertir pesos: bruto={peso_bruto}, tara={peso_tara}")
            peso_neto = 0
        ...
        # Almacenar en la base de datos
        from db_operations import store_pesaje_neto
        store_pesaje_neto(datos_pesaje)
        
        # Actualizar datos en sesión
        session['peso_tara'] = peso_tara
        session['peso_neto'] = peso_neto
        session['tipo_pesaje_neto'] = tipo_pesaje
        session['fecha_pesaje_neto'] = fecha_pesaje
        session['hora_pesaje_neto'] = hora_pesaje
        
        return jsonify({
            'success': True,
            'message': f'Peso neto registrado: {peso_neto}kg',
            'peso_neto': peso_neto
        })
        
    except Exception as e:
        logger.error(f"Error en registrar_peso_neto: {str(e)}")
        return jsonify({
            'success': False,
            'message': f'Error al registrar peso neto: {str(e)}'
        })
...
def iniciar_procesamiento(url_guia):
    ...
    try:
        ...
        # Obtener la ruta de las fotos
        guia_fotos_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'fotos', url_guia)
        if not os.path.exists(guia_fotos_dir):
            logger.error(f"No se encontró directorio de fotos para {url_guia}")
            return jsonify({
                'status': 'error',
                'message': f"No se encontró directorio de fotos para {url_guia}"
            }), 404
        
        # Leer las imágenes en el directorio
        fotos_paths = []
        ...
        def process_thread():
            try:
                ...
                process_images_with_roboflow(url_guia, fotos_paths, guia_fotos_dir, json_path)
                ...
                processing_status[url_guia] = {
                    ...
                    'redirect_url': f'/ver_resultados_automaticos/{url_guia}'
                }
            except Exception as e:
